...
fn should_run_e2e() -> bool {
    std::env::var("CLAWFORM_E2E_CLAUDE")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
}

fn ensure_claude_auth() -> Result<()> {
    let out = Command::new("claude")
        .args(["auth", "status", "--text"])
        .output()
        .context("failed to run 'claude auth status --text'")?;
...
#[test]
#[serial]
fn claude_smoke_apply_creates_output() -> Result<()> {
    if !should_run_e2e() {
        eprintln!("skipping claude e2e: set CLAWFORM_E2E_CLAUDE=1 to run");
        return Ok(());
    }
...
#[test]
#[serial]
fn claude_second_apply_runs_and_prints_files_summary() -> Result<()> {
    if !should_run_e2e() {
        eprintln!("skipping claude e2e: set CLAWFORM_E2E_CLAUDE=1 to run");
        return Ok(());
    }
...
#[test]
#[serial]
fn claude_apply_can_create_multiple_files() -> Result<()> {
    if !should_run_e2e() {
        eprintln!("skipping claude e2e: set CLAWFORM_E2E_CLAUDE=1 to run");
        return Ok(());
    }
